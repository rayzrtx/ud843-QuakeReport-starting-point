...
/**
 * Created by raylinares on 1/4/18.
 */
...
public class QueryUtils  {
    ...
    private static URL createUrl(String stringUrl) {
        URL earthquakeDataURL = null;
        try {
            earthquakeDataURL = new URL(stringUrl);
        } catch (MalformedURLException e) {
            Log.e(LOG_TAG, "Error with creating URL ", e);
        }
        return earthquakeDataURL;
    }

    /**
     * Make an HTTP request to the given URL and return a String as the response.
     */
    private static String makeHttpRequest(URL earthquakeDataURL) throws IOException {
        String jsonResponse = "";

        // If the URL is null, then return early.
        if (earthquakeDataURL == null) {
            return jsonResponse;
        }
        ...
        try {
            urlConnection = (HttpURLConnection) earthquakeDataURL.openConnection();
    ...
    public static ArrayList<Earthquake> extractFeatureFromJSON(String earthquakeJSON) {
        ...
        try {
            ...
            //For each earthquake in the featuresArray, create an {@link Earthquake} object
            for (int i = 0; i < featuresArray.length(); i++){
                //Get the JSON object in the features array at index i
